...
const result = lines.reduce((acc, line) => {
  Object.keys(wordsToNumbers).forEach((word) => {
    ...
    line = line.replace(regex, `${wordsToNumbers[`${word}`]}`);
  });

  let firstNumber = line.match(/^.*?(\d)/);
  let lastNumber = line.match(/.*(\d).*$/);

  if (firstNumber?.length === 2 && lastNumber?.length === 2) {
    let sum = parseInt(`${firstNumber[1]}${lastNumber[1]}`);
    return acc + sum;
  }
